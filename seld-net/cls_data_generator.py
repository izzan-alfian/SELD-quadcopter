...
class DataGenerator(object):
    def __init__(
        ...
        self._batch_size = batch_size   # Number of sequence for a single batch
        self._seq_len = seq_len         # Number of frames for a single sequence
        ...
        self._nb_frames_file = None     # Assuming number of frames in feat files are the same          # Number of frame inside a single file
        self._feat_len = None                                                                           # Number of frequency bins per channel
        ...
        self._batch_seq_len = self._batch_size*self._seq_len                                            # Number of frames for a single batch
        self._circ_buf_feat = None
        self._circ_buf_label = None
        
        # A single epoch contains all frames                                                            # Number of frames inside single batch (240×5166)÷(512×16)
        self._nb_total_batches = int(np.floor((len(self._filenames_list) * self._nb_frames_file /
                                               float(self._seq_len * self._batch_size))))               # Number of batches for a single epoch
    ...
    def get_data_sizes(self):
        ...
        return feat_shape, label_shape                  # (16, 16, 512, 256)        # ((16, 512, 1), (16, 512, 3))
    ...
    def _get_label_filenames_sizes(self):
        ...
        self._feat_len = temp_feat.shape[1] // self._2_nb_ch                                # 256
    ...
    def generate(self):
        ...
        while 1:
            ...
            for i in range(self._nb_total_batches):
                ...
                while len(self._circ_buf_feat) < self._batch_seq_len:                                       # Loop eventually ends after 1 batch worth of frame
                    temp_feat = np.load(os.path.join(self._feat_dir, self._filenames_list[file_cnt]))       # loads a single file of feature
                    temp_label = np.load(os.path.join(self._label_dir, self._filenames_list[file_cnt]))     # loads a single file of label
                ...
                feat = np.zeros((self._batch_seq_len, self._feat_len * self._2_nb_ch))                      # (8192, 2048)
                label = np.zeros((self._batch_seq_len, self._label_len))                                    # (8192, 33)
                for j in range(self._batch_seq_len):
                    feat[j, :] = self._circ_buf_feat.popleft()                                              # (8192, 2048)
                    label[j, :] = self._circ_buf_label.popleft()                                            # (8192, 33)
                feat = np.reshape(feat, (self._batch_seq_len, self._feat_len, self._2_nb_ch))               # (8192, 256, 8) (frames per batch, frequency bins, 2 * channels)

                # Split to sequences
                feat = self._split_in_seqs(feat)                                                            # (16, 512, 256, 8) (sequences per batch, frames per sequence, frequency bins, 2 * channels)
                feat = np.transpose(feat, (0, 3, 1, 2))                                                     # (16, 8, 512, 256) (sequences per batch, 2 * channels, frames per sequence, frequency bins)
                label = self._split_in_seqs(label)                                                          # (16, 512, 33) (sequences per batch, SED(11) + DOA (azi + ele = 11 + 11))

                if self._azi_only:
                    # Get Cartesian coordinates from azi/ele
                    azi_rad = label[:, :, self._nb_classes:2 * self._nb_classes] * np.pi / 180              # (16, 512, 11-22)
                    x = np.cos(azi_rad)                                                                     # (16, 512, 11)
                    y = np.sin(azi_rad)                                                                     # (16, 512, 11)
