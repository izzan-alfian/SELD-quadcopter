...
// I2S Configuration
#define A0_I2S_MIC_PORT I2S_NUM_0 // I2S port number
#define A1_I2S_MIC_PORT I2S_NUM_1 // I2S port number
...
#define A0_I2S_MIC_SERIAL_CLOCK GPIO_NUM_16  // SCK / BCLK
#define A0_I2S_MIC_WORD_SELECT  GPIO_NUM_17 // WS / LRCK
#define A0_I2S_MIC_SERIAL_DATA  GPIO_NUM_18 // SD / DIN (Data IN for ESP32)

#define A1_I2S_MIC_SERIAL_CLOCK GPIO_NUM_9  // SCK / BCLK
#define A1_I2S_MIC_WORD_SELECT  GPIO_NUM_10 // WS / LRCK
#define A1_I2S_MIC_SERIAL_DATA  GPIO_NUM_11 // SD / DIN (Data IN for ESP32)
...
void i2s_mic_task(void *parameter) {
    ...
    int32_t* A1_i2s_read_buffer = (int32_t*)malloc(I2S_READ_BUFFER_SIZE_BYTES);

    if (!A0_i2s_read_buffer) {
        Serial.println("Failed to allocate memory for A0 I2S read buffer");
        vTaskDelete(NULL);
        return;
    }
    if (!A1_i2s_read_buffer) {
        Serial.println("Failed to allocate memory for A1 I2S read buffer");
        vTaskDelete(NULL);
        return;
    }
    ...
    if (!A0_serial_write_buffer) {
        Serial.println("Failed to allocate A0 memory for serial write buffer");
        free(A0_i2s_read_buffer);
        vTaskDelete(NULL);
        return;
    }
    if (!A1_serial_write_buffer) {
        Serial.println("Failed to allocate A1 memory for serial write buffer");
        free(A0_i2s_read_buffer);
        vTaskDelete(NULL);
        return;
    }
    ...
    free(A0_i2s_read_buffer);
    free(A1_i2s_read_buffer);
    free(A0_serial_write_buffer);
    free(A1_serial_write_buffer);
    vTaskDelete(NULL);
}
...
void setup() {
    ...
    i2s_pin_config_t A0_pin_config = {
        .bck_io_num = A0_I2S_MIC_SERIAL_CLOCK,
        .ws_io_num = A0_I2S_MIC_WORD_SELECT,
        .data_out_num = I2S_PIN_NO_CHANGE, // Not used for RX
        .data_in_num = A0_I2S_MIC_SERIAL_DATA
    };


    i2s_config_t A1_i2s_config = {
    ...
    // Configure I2S pins
    i2s_pin_config_t A1_pin_config = {
        .bck_io_num = A1_I2S_MIC_SERIAL_CLOCK,
        .ws_io_num = A1_I2S_MIC_WORD_SELECT,
        .data_out_num = I2S_PIN_NO_CHANGE, // Not used for RX
        .data_in_num = A1_I2S_MIC_SERIAL_DATA
    };

    esp_err_t err;

    // Install and start I2S driver
    err = i2s_driver_install(A0_I2S_MIC_PORT, &A0_i2s_config, 0, NULL);
    if (err != ESP_OK) {
        Serial.printf("Failed to install A0 I2S driver: %d\n", err);
        return;
    }

    err = i2s_set_pin(A0_I2S_MIC_PORT, &A0_pin_config);
    if (err != ESP_OK) {
        Serial.printf("Failed to set A0 I2S pins: %d\n", err);
        return;
    }

    err = i2s_driver_install(A1_I2S_MIC_PORT, &A1_i2s_config, 0, NULL);
    if (err != ESP_OK) {
        Serial.printf("Failed to install A1 I2S driver: %d\n", err);
        return;
    }

    err = i2s_set_pin(A1_I2S_MIC_PORT, &A1_pin_config);
    if (err != ESP_OK) {
        Serial.printf("Failed to set A1 I2S pins: %d\n", err);
        return;
    }
